...
public class BlockGame3 {
    ...
    static class MyFrame extends JFrame {
        ...
        static class Ball {
            ...
            int y = ((CANVAS_HEIGHT / 3)*2) - ((BALL_HEIGHT / 3)*2);
            int width = BALL_WIDTH;
            int height = BALL_HEIGHT;


            Point getRightTop() {
                return new Point(x + (BALL_WIDTH - 5), y);
            }

            Point getRight1() {
                return new Point(x + BALL_WIDTH, y + 5);
            }

            Point getRight2() {
                return new Point(x + BALL_WIDTH, y + (BALL_HEIGHT - 5));
            }

            Point getRightBottom() {
                return new Point(x + BALL_WIDTH - 5, y + BALL_HEIGHT);
            }

            Point getLeftBottom() {
                return new Point(x + 5, y + BALL_HEIGHT);
            }

            Point getLeft2() {
                return new Point(x, y + BALL_HEIGHT - 5);
            }
        ...
        public void checkCollision() {
            if (dir == 0) { //0 : Up-Right
                ...
                if (ball.y > CANVAS_HEIGHT - BALL_HEIGHT - BALL_HEIGHT) { //wall bottom
                    ...
                    ball.y = ((CANVAS_HEIGHT / 3) * 2) - ((BALL_HEIGHT / 3) * 2);
                    score = 0;
                }
                if (ball.x > CANVAS_WIDTH - BALL_WIDTH) { //wall right
                    dir = 3;
                    ...
                    if (block.isHidden == false) {
                        if (dir == 0) { //0 : Up-Right
                            if (duplRect(new Rectangle(ball.x, ball.y, ball.width, ball.height),
                                ...
                                if (ball.getRightTop().x >= block.x && ball.getRightTop().x <= block.x + block.width) {
                                    //block bottom collision
                                    dir = 1;
                                } else if (ball.getRight1().x >= block.x && ball.getRight1().x <= block.x + block.width) {
                            ...
                            if (duplRect(new Rectangle(ball.x, ball.y, ball.width, ball.height),
                                    new Rectangle(block.x, block.y, block.width, block.height))) {
                                if (ball.getRightBottom().x >= block.x && ball.getRightBottom().x <= block.x + block.width) {
                                    //block top collision
                                    dir = 0;
                                } else if (ball.getRight2().x >= block.x && ball.getRight2().x <= block.x + block.width) {
                                    //block left collision
                                    dir = 3;
                                }
                                block.isHidden = true;
                            ...
                            if (duplRect(new Rectangle(ball.x, ball.y, ball.width, ball.height),
                                    new Rectangle(block.x, block.y, block.width, block.height))) {
                                if (ball.getLeftTop().x >= block.x && ball.getLeftTop().x <= block.x + block.width) {
                                    //block bottom collision
                                    dir = 3;
                                } else if (ball.getLeft1().x >= block.x && ball.getLeft1().x <= block.x + block.width) {
                            ...
                            if (duplRect(new Rectangle(ball.x, ball.y, ball.width, ball.height),
                                    new Rectangle(block.x, block.y, block.width, block.height))) {
                                if (ball.getLeftBottom().x >= block.x && ball.getLeftBottom().x <= block.x + block.width) {
                                    //block top collision
                                    dir = 2;
                                } else if (ball.getLeft2().x >= block.x && ball.getLeft2().x <= block.x + block.width) {
                                    //block right collision
                                    dir = 1;
                                }
                                block.isHidden = true;
